...
import { BookOpen, ArrowLeft, Video, Mic, X, Edit, Trash2, Save } from "lucide-react";
...
export function StoryDetail() {
  ...
  const fetchStoryDetail = async (storyId: string) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(response.message || "获取故事详情失败");
      }
    } catch (err) {
      setError("网络错误，请稍后重试");
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await api.story.delete(id, token);
      if (response.success) {
        navigate("/");
      } else {
        setError(response.message || "删除故事失败");
      }
    } catch (err) {
      setError("网络错误，请稍后重试");
  ...
  if (!story) {
    return (
      <div className="min-h-screen bg-indigo-950 flex items-center justify-center">
        <p className="text-indigo-200">故事不存在</p>
      </div>
    );
  }
            ...
            {editing ? (
              ...
              <>
                ...
                <div className="flex items-center gap-2 mb-6">
                  <span className="text-indigo-300 text-sm">阅读时长：5分钟</span>
                </div>
                ...
                <div className="bg-indigo-800/30 border border-indigo-700 rounded-xl p-6">
                  ...
                  <p className="text-indigo-200 leading-relaxed">{story.content}</p>
                </div>
              </>
            )}
          </div>
